...
<div class="tvseriesheading">
	{if $rage[0].imgdata != ""}<img class="shadow" width="185" height="278" alt="{$rage[0].releasetitle} Logo" src="{$smarty.const.WWW_TOP}/getimage?type=tvrage&amp;id={$rage[0].ID}" />{/if}
...
<h2>{if $seriesgenre != ''}{$seriesgenre}{/if}</h2>
</h1>
	...
	<p>
		...
		{if $seriesdescription != ''}<span class="descinitial">{$seriesdescription|escape:"htmlall"|nl2br|magicurl}</span>{/if}
...
<span>
{if $myshows.ID != ''}
<a href="{$smarty.const.WWW_TOP}/myshows/edit/{$rage[0].rageID}?from={$smarty.server.REQUEST_URI|escape:"url"}" class="myshows" rel="edit" name="series{$rage[0].rageID}" title="Edit Categories for this show">Edit</a> | 
...
<div class="nzb_multi_operations">
	<div style="padding-bottom:10px;" >
		{if $rage|@count == 1 && $isadmin}<a href="{$smarty.const.WWW_TOP}/admin/rage-edit.php?id={$r.ID}&amp;action=update&amp;from={$smarty.server.REQUEST_URI|escape:"url"}">Update From Tv Rage</a> | {/if}
...
{foreach $seasons as $seasonnum => $season name="seas"}
  <li {if $smarty.foreach.seas.first}class="active"{/if}><h2><a id="seas_{$seasonnum}" title="View Season {$seasonnum}" href="#{$seasonnum}">{$seasonnum}</a></h2></li>
{/foreach}
</ul>

	{foreach $seasons as $seasonnum => $season name=tv}
	<table style="width:100%; {if !$smarty.foreach.tv.first}display:none;{/if}" class="tb_{$seasonnum} data highlight icons" id="browsetable">
		<tr>
